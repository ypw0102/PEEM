...
def calculate_overlap(list1, list2,is_self):
    cnt = 1e-10
    F_res=0
    for i in range(len(list1)):
        for x in range(len(list1[i])):
            for y in range(len(list2[i])):
                if x==y and is_self:continue
                res = cal_me(list1[i][x], list2[i][y])
                cnt+=1
                F_res += res['F']
    return F_res/cnt
...
def cal_me(in_s, ou_s):

    predictCount = sum(ou_s)
    correctCount = sum(in_s)
    agreementCount = sum([a * b for (a, b) in zip(in_s, ou_s)])
    ...
    F1m = 2 * R1 * P1 / (P1 + R1+1e-10)
    return {"P": P1, "R": R1, "F": F1m}
...
modes = ["human","weem"]
for mode in modes:
        to_store_dir = "exp_result/multirc_model-bili-{}_sample-bili-{}_mode-{}_subname-{}.json".format(model_bili,sample_bili,mode,sub_name)
        print("************************")
        print(to_store_dir)
        ...
        if True:
            ...
            for model in all_models_:
                ...
                for i in range(len(datas[model])):
                    ...
                    rate = vote(datas[model][i]['ans_pre_human']['predicts'], mode)
                    all_rate[model][key] = rate
                    all_rate['Alignment'][key] = [datas[model][i]['ans_pre_human']['answer']]
                    all_rate['Human'][key] = [datas[model][i]['ans_pre_human']['human']]
            keys = None
            ...
            for i, attr1 in enumerate(attributes):
                for j, attr2 in enumerate(attributes):
                    ...
                    overlap_matrix_raw[attr1][attr2] = calculate_overlap(all_rate_[attr1], all_rate_[attr2],attr2==attr1)

            for ites in trange(500):
                all_models=random.sample(all_models_,int(model_bili*len(all_models_)))+["Human"]
                # all_models=all_models_+["Human"]
                ...
                for i, attr in enumerate(attributes):
                    overlap_matrix[-1, i] = calculate_overlap(all_rate['Alignment'], all_rate[attr],False)

                # strategy
                if mode == "ensemble" or mode == "base":
                    pass
                elif mode == "human":
                    overlap_matrix[-2] = overlap_matrix[-3]
                    ...
                    no_drop_list = [i for i in range(len(attributes)) if i not in drop_list]
                    weights = np.zeros_like(overlap_matrix[-2])
                    for i in range(len(attributes)):
                        if i not in drop_list:
                            weights+=overlap_matrix[i]
                    weights /= len(no_drop_list)
                    if len(no_drop_list)==1:
                        for i in range(min(3,len(attributes)//2)):
                            weights[no_drop_list[0]]+=overlap_matrix[drop_list[-i-1]][no_drop_list[0]]
                        weights[no_drop_list[0]]/=(min(3,len(attributes)//2)+1)
                ...
                cur_res = get_corrs(overlap_matrix[-1][:-1], overlap_matrix[-2][:-1])
                pe_avg += cur_res['pe']
                sp_avg += cur_res['sp']
                ke_avg += cur_res['ke']
                cnt_avg += 1
